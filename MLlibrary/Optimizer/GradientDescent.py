...
if __name__ == "__main__":
    ...
    x_data = np.asarray([list(row[:2]) for row in data]) # each element is [1, experience, salary], note '1' is for intercept term
    x_data[:,0] = x_data[:,0] / max(x_data[:,0])
    x_data[:,1] = x_data[:,1] / max(x_data[:,1])
    y_data = np.asarray([row[2] for row in data]).reshape((-1,1))        # each element is paid_account

    w = tf.get_variable('weight', [2,1], initializer=tf.truncated_normal_initializer())
   ...
   # print(tf.trainable_variables())
    ...
    loss = cf.cross_entropy(model, y)
    optmz = GradientDescent()
    
    with tf.Session() as sess:
        init = tf.global_variables_initializer()
        sess.run(init)
        l = sess.run(linear, feed_dict={x : x_data, y : y_data})
        m = sess.run(model, feed_dict={x : x_data, y : y_data})
